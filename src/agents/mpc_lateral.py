...
class MPC_Lateral():
    ...
    def _setup_mpc(self):
        ...
        heading = model.set_variable(var_type='_x', var_name='psi')          
        ...
        heading_ref = model.set_variable(var_type='_tvp', var_name='heading_ref')  
        d_min = model.set_variable(var_type='_tvp', var_name='d_min')
        d_max = model.set_variable(var_type='_tvp', var_name='d_max')

        # --- 3. Equations of Motion ---
        model.set_rhs('d', v_ext * ca.sin(heading_ref))
        model.set_rhs('psi', (v_ext / self.wheelbase_L) * ca.tan(steer))
        model.setup()
        ...
        mpc.set_rterm(steer=self.R_steer)

        # --- 5. Constraints (Road Boundaries & Physics) ---
        
        mpc.bounds['lower', '_u', 'steer'] = self.min_steer_rad
        mpc.bounds['upper', '_u', 'steer'] = self.max_steer_rad
        mpc.bounds['lower', '_x', 'heading'] = self.min_yaw_rad
        mpc.bounds['upper', '_x', 'heading'] = self.max_yaw_rad
